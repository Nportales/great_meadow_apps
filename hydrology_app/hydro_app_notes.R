# 5/28/26 ----------------------------------------------------------------------
## app prior to moving placement of significance testing notes------------------
...
server <- function(input, output, session) {
  ...
  <div style='background-color:#f9f9f9; padding:10px; border-left:4px solid #1B365D; margin-bottom:10px; font-size:13px;'>
    <strong>Significance testing:</strong><br>
    • Compares the grand means of each statistic between wetlands for all selected years and sites. Highlighted variables show <span style='background-color: #fff3cd; padding: 2px 4px; border-radius: 3px; color: #856404; font-weight: bold;'>*statistically significant</span> differences (p < 0.05) between wetlands.<br>
...
## app prior to the stacking of wl stats plots----------------------------------
        ...
        if (sig_row$significant) {
          ...
          div(
            ...
            div(
              style = 'background-color:#fff3cd; padding:5px; border-left:4px solid #856404; border-radius:4px;',
              HTML(sprintf(
                "<strong style='color:#856404; font-size:13px;'>%s Statistically Significant (p = %.3f)</strong><br>
        ...
        Significance testing compares grand means between Great Meadow and Gilmore Meadow wetlands.
          ...
          div(
            ...
            div(
              style = 'background-color:#f5f5f5; padding:5px; border-left:4px solid #6c757d; border-radius:4px;',
              HTML(sprintf(
                "<strong style='color:#495057; font-size:13px;'>Not Statistically Significant (p = %.3f)</strong><br>
        <span style='color:#666666; font-size:13px; font-style:italic;'>
        Significance testing compares grand means between Great Meadow and Gilmore Meadow wetlands.
        </span>",
                sig_row$p_value
    ...
    content = function(file) {
      ...
      if (!is.null(sig_results)) {
        ...
        if (nrow(sig_row) > 0 && !is.na(sig_row$p_value)) {
          ...
          if (sig_row$significant) {
            label_text <- sprintf("p = %.3f *", sig_row$p_value)
          } else {
            label_text <- sprintf("p = %.3f (ns)", sig_row$p_value)
          }
...
#####---------------------------------------------------------------------------
## app prior to the navigation bar addition-------------------------------------
...
## app before wetland chooser in time series plot section
...
#### COPY OF PREVIOUS HYDROLOGY DASHBOARD 
...
## added significance note for cases that are not significant too
## significance testing - plots
## trying new t-test -> paired t-test
          ...
          return(tagList(
            ...
            div(
              ...
              div(
                ...
                <ul style='margin-top:8px; margin-bottom:5px; color:#333333; font-size:12px;'>%s</ul>",
                  as.character(icon("asterisk")),
                  paste(sapply(sig_display_names, function(name) paste0("<li>", name, "</li>")), collapse = "")
                ))
              )
            )
          ))
      ...
      if (nrow(sig_row) > 0 && sig_row$significant) {
        div(
          ...
          Significance testing compares grand means between Great Meadow and Gilmore Meadow wetlands.
              ...
              as.character(icon("asterisk")),  # <-- This was missing!
              sig_row$p_value
            ))
          )
        )
...
ui <- page_fluid(
  ...
  # THIRD SECTION: Water Level Statistics Time Series
  div(class = "content-section",
      layout_sidebar(
        sidebar = sidebar(
          class = "sidebar-custom", width = 300,
          ...
          br(),
          div(style = "padding: 10px; background-color: #e8f4f8; border-radius: 8px; border-left: 4px solid #3498db;",
              p(icon("info-circle"), " This plot shows the average of each statistic across all sites within each wetland for each year.", 
                style = "margin: 0; font-size: 0.9rem; color: #2c3e50;")),
          
          br(),
          div(style = "padding: 10px; background-color: #fff3cd; border-radius: 8px; border-left: 4px solid #f39c12;",
              p(icon("lightbulb"), strong("Dashed lines"), "represent the grand mean (average across all years).", 
...
server <- function(input, output, session) {
  ...
  # In the server function, add this:
  ...
  output$download_timeseries <- downloadHandler(
    ...
    content = function(file) {
      ...
      p <- if (input$selected_metric == "WL_sd") {
        ggplot(wl_test, aes(x = year, y = WL_sd_avg, color = wetland, group = wetland)) +
          ...
          geom_hline(data = grand_means,
                     ...
                     linetype = "dashed", linewidth = 1) +
          scale_color_manual(values = c("Great Meadow" = "black", "Gilmore Meadow" = "grey67")) +
          scale_x_continuous(breaks = seq(min(wl_test$year), max(wl_test$year), by = 1)) +
          labs(title = "Within-Site Water Level Variability Over Time",
               x = "Year", y = metric_label, color = "Wetland") +
          theme_minimal() +
          theme(plot.title = element_text(hjust = 0.5, size = 16, face = "bold"),
                axis.text.x = element_text(angle = 45, hjust = 1))
      } else {
        plot_wl_metric(wl_test, grand_means, input$selected_metric, 
                       metric_label, paste(metric_label, "Over Time"))
      }
...
## alternative significance testing --------------------------------------------
...
get_significance_label <- function(metric, sig_results) {
  if (is.null(sig_results)) return(NULL)
  
  row <- sig_results %>% filter(variable == metric)
  
  if (nrow(row) == 0 || is.na(row$p_value)) return(NULL)
  
  if (row$significant) {
    return(paste0("★ Significant (p = ", signif(row$p_value, 3), ")"))
  } else {
    return(paste0("p = ", signif(row$p_value, 3)))
  }
}
...
server <- function(input, output, session) {
  ...
  output$stat_timeseries <- renderPlot({
    ...
    # get significance results
    sig_results <- significance_results()
    sig_label <- get_significance_label(metric, sig_results)
    ...
    if (!is.null(sig_label)) {
      p <- p +
        annotate(
          ...
          hjust = 1.1, vjust = 1.5,
          size = 5,
          fontface = ifelse(grepl("Significant", sig_label), "bold", "plain"),
          color = ifelse(grepl("Significant", sig_label), "#e74c3c", "black")
        )
    }
...
## testing out new reactive graphs for water level stats 
...
# Reverse mapping for plots
DISPLAY_TO_VAR <- setNames(names(VAR_MAPPING), VAR_MAPPING)
...
plot_wl_metric <- function(data, grand_data, metric, y_label, title) {
  ...
  ggplot(data, aes(x = year, y = .data[[avg_col]], color = wetland, group = wetland)) +
    ...
    # error bars (only if SD exists and metric is not WL_sd)
    {if(metric != "WL_sd") {
      geom_errorbar(
        aes(ymin = .data[[avg_col]] - .data[[sd_col]],
            ymax = .data[[avg_col]] + .data[[sd_col]]),
      ...
      layout_sidebar(
        ...
        card(
          ...
          card_header(class = "bg-info text-white", 
                      "Time Series of Selected Statistic",
                      tooltip(icon("info-circle"), 
                              "Click on a column header in the table above to view its time series")),
          div(style = "padding: 15px;",
              uiOutput("stat_selection_info"),
              plotOutput("stat_timeseries", height = "500px"))
        )
      )
  ),
...
## NEW ## ----------------------------------------------------------------------
  # In the server function, add these:
  
  # Track clicked column in the statistics table
  clicked_stat <- reactiveVal(NULL)
  
  # Observe table clicks
  observeEvent(input$wl_stats_columns_selected, {
    req(input$wl_stats_columns_selected)
    
    # Get the column index
    col_idx <- input$wl_stats_columns_selected
    
    # Get column name from filtered_stats
    col_names <- names(filtered_stats())
    
    if (col_idx > 0 && col_idx <= length(col_names)) {
      selected_col <- col_names[col_idx]
      
      # Only process if it's a statistic column (not Year, Site, or Wetland)
      if (!selected_col %in% c("Year", "Site", "Wetland")) {
        clicked_stat(selected_col)
      }
    }
  })
  
  # Display info about selected statistic
  output$stat_selection_info <- renderUI({
    if (is.null(clicked_stat())) {
      div(style = "padding: 10px; background-color: #e8f4f8; border-radius: 8px; border-left: 4px solid #3498db; margin-bottom: 15px;",
          p(icon("hand-pointer"), " Click on a column header in the table above to view its time series over years.", 
            style = "margin: 0; font-size: 0.9rem; color: #2c3e50;"))
    } else {
      div(style = "padding: 10px; background-color: #d4edda; border-radius: 8px; border-left: 4px solid #28a745; margin-bottom: 15px;",
          p(icon("chart-line"), strong("Currently viewing:"), clicked_stat(), 
            style = "margin: 0; font-size: 0.9rem; color: #155724;"))
    }
  })
  ...
  output$stat_timeseries <- renderPlot({
    ...
    # Map display name back to variable name
    var_name <- DISPLAY_TO_VAR[clicked_stat()]
    
    # Handle special case for WL_sd
    if (clicked_stat() == "SD Water Level (cm)") {
      ggplot(wl_test, aes(x = year, y = WL_sd_avg, color = wetland, group = wetland)) +
        geom_line(linewidth = 1.2) +
        geom_point(size = 3) +
        ...
        scale_x_continuous(
          breaks = seq(min(wl_test$year), max(wl_test$year), by = 1)
        ) +
        
        labs(title = paste("Within-Site Water Level Variability Over Time"),
             x = "Year",
             y = "Mean Site-Level SD (cm)",
             color = "Wetland") +
      ...
      # Use the plotting function for other metrics
      plot_wl_metric(
        wl_test,
        grand_means,
        metric = var_name,
        y_label = clicked_stat(),
        title = paste(clicked_stat(), "Over Time")
      )
    } else {
      # Fallback: show a message
      ggplot() + 
        annotate("text", x = 0.5, y = 0.5, 
                 label = "Click on a statistic column to view its time series", 
                 size = 6) +
        theme_void()
    }
  })
